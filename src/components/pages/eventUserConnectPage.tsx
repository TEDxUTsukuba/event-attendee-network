...
export default function EventUserConnectPage({ eventData, targetUserId }: { eventData: EventData, targetUserId: string }) {
  ...
  const onSubmit = async () => {
    const isTargetQuestionValid = targetUserData.question.trim() !== "";

    if (!isTargetQuestionValid) {
      setErrors((prevErrors) => ({ ...prevErrors, e_answer: "答えを入力してください" }));
      return;
    }

    // 正誤判定
    // if (answer) {
    //   setErrors((prevErrors) => ({ ...prevErrors, e_answer: "正解" }));
    // } else {
    //   setErrors((prevErrors) => ({ ...prevErrors, e_answer: "不正解" }));
    // }
      ...
      <Card className="max-w-md mx-auto w-full">
        ...
        <CardContent className="flex flex-col gap-4">
          ...
          <div>
            ...
            {errors.e_answer && <p className="text-red-500">{errors.e_answer}</p>}
          </div>
